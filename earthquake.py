...
# Read the 'Magnitude' column without skipping rows
data = pd.read_csv(file_path, usecols=['Longitude'])

# Extract the 'Magnitude' column as a numpy array
signal = data['Longitude'].values
# Define the training and testing data
train_data, temp_data = train_test_split(signal, test_size=0.2, shuffle=False)
...
model.add(keras.layers.LSTM(units=32, return_sequences=True, input_shape=(X_train.shape[1], 1)))
...
history=model.fit(X_train, y_train, batch_size=32, epochs=200, shuffle=False, verbose=1,validation_data=(X_validation, y_validation))
